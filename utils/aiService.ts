...
export function getAIConfig(): { provider: 'gemini' | 'openai'; apiKey: string } {
  const provider = (localStorage.getItem('andopages_ai_provider') || 'gemini') as 'gemini' | 'openai';
  const apiKey = localStorage.getItem('andopages_ai_key') || (provider === 'gemini' ? DEFAULT_GEMINI_KEY : '');
  return { provider, apiKey };
}

// Guardar configuración en localStorage
export function saveAIConfig(provider: 'gemini' | 'openai', apiKey: string) {
...
export async function analyzeProductWithAI(
  ...
  if (!apiKey) {
    throw new Error(
      `La API Key para ${provider === 'gemini' ? 'Gemini' : 'OpenAI'} no está configurada. Por favor, haz clic en el ícono de ajustes en el asistente para configurarla.`
    );
  }
